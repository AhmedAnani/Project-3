...
public class GoogleCalendarService : IGoogleCalendarService
{
    ...
    public async Task<string?> ScheduleTripEventAsync(string googleAccessToken, TripBucketItem trip)
    {
        try
        {
        ...
        {
            _logger.LogError(ex, "Failed to schedule Google Calendar event for trip {TripId}.", trip.Id);
            return null;
        }
    }

    public async Task<bool> UpdateTripEventAsync(string googleAccessToken, TripBucketItem trip)
    {
        try
        {
            ...
            var request = service.Events.Update(calendarEvent, "primary", trip.GoogleEventId);
            await request.ExecuteAsync();
            return true;
        }
        catch (HttpRequestException ex) when (ex.Message.Contains("404") || ex.Message.Contains("410"))
        {
            _logger.LogWarning("Google Calendar event {GoogleEventId} not found (404/410). Treating as deleted.", trip.GoogleEventId);
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to update Google Calendar event for trip {TripId}.", trip.Id);
            return false;
        }
    }

    public async Task<bool> DeleteTripEventAsync(string googleAccessToken, string googleEventId)
    {
        try
        {
